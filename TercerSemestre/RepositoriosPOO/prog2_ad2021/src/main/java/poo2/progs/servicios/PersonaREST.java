...
Cuando vayas a comparar un String con otro, evita usar el comparador "==", recuerda que estas comparando OBJETOS, el operador correcto para comparar
Strings es el metodo equals.. 

no olvides anteponer el "@" a las anotaciones

cuidado con las llaves, tuviste muchas malas colocaciones sobre todo en los try-catch

los retornos de los metodos NO son boolean, recuerda que el servidor recibe una cadena con true o false, por lo tanto debes retornar un String.

cuidado con los nombres de los campos de la base de datos, tuviste varios errores en los querys en esas partes ejemplo de una correcion que hize... 
(e.email_concursante2    era  e.emailConcursante2)

no olvides los return despues de las validaciones.
*/
...
public class PersonaREST extends RESTAbstracto<Persona>{
    ...
    } //Entidad entre 1 y 32

    private boolean municipioValido(long id_municipio){ //Verifica si el municipio existe
    ...
    private boolean institucionValida(long id_institucion) { //Verifica que la institucion existe
        boolean resultado = true;
        try {
    ...
    public String agregaPersona(Persona dato) {
        //Entidad debe ser valida
        //Municipio debe existir
        //Institucion debe existir
        //Limitar sexo a M o F
        //Limitar tipo a Estudiante o Profesor
        ...
        if (entidadValida(dato.getIdEntidadPersona())
                ...
                && (dato.getTipoPersona().equals("Profesor") || dato.getTipoPersona().equals("Estudiante")) ) {
    ...
    public String actualizaPersona(@PathParam("emailpersona")String email, Persona dato) {
        String resultado = "false";
        // Mismas validaciones que agregar... pero asegurarse que el email enviado y el del dato coincidan
        if (entidadValida(dato.getIdEntidadPersona())
                && municipioValido(dato.getIdMunicipioPersona())
                && institucionValida(dato.getIdInstitucionPersona())
    ...
    private boolean noHayRegistrosAsociados(String email){
        // Verificar que en las tablas donde el email es llave secundaria
        //No existan registros con el dato a eliminar
    ...
    public String obtenCorreosDeInstitucion(@PathParam("idinst") Long idInstitucion, @PathParam("tipopersona") String tipo){
        //Hacer una consulta para sacar los emails de la institucion y el tipo de persona indicados
        //Concatenar una String separadas por ";" para el retorno.
